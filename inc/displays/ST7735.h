...
#define RES_X 160	//Screen X axis resolution
#define RES_Y 128	//Screen Y axis resolution


#define _CS_EN *_CSP &= ~_CSM
#define _CS_DIS *_CSP |= _CSM

#define _DC_CMD *_DCP &= ~_DCM
#define _DC_DAT *_DCP |= _DCM;

#define _RS_EN *_RSP &= ~_RSM
#define _RS_DIS *_RSP |= _RSM

#define _BL_DIS *_BLP &= ~_BLM
#define _BL_EN *_BLP |= _BLM
...
class Display
{
	public:
		Display(void);
		void DInit(uint8_t cs, uint8_t dc, uint8_t rst, uint8_t bl = 255);
	...
	private:
		...
		void _Send_Command(uint8_t Command, uint8_t Data);
		void _Send_Command(uint8_t Command, uint8_t Data1, uint8_t Data2);
		void _Send_Data(uint8_t Data);
		void _SetWriteArea(uint8_t X1, uint8_t Y1, uint8_t X2, uint8_t Y2);
		...
		uint16_t _FGColour = 0xFFFF , _BGColour = 0x0000;
		int16_t _X1Pos = 0, _Y1Pos = 0;
		volatile uint8_t *_CSP, *_DCP, *_RSP, *_BLP;
		uint8_t _CSM, _DCM, _RSM, _BLM;
		boolean _blFlag = false;
		uint8_t _res_x = RES_X, _res_y = RES_Y;
